...
Description:
  Checks whether AWS DMS Replication Instance has ingress rule present in attached Security Group
...
Scenarios:
  Scenario: 1
    Given: No AWS DMS Replication Instance exists
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for dms_instance_details in dms_instances:
        attached_security_group_ids = []
        attached_security_groups = dms_instance_details['VpcSecurityGroups']
        for sg in attached_security_groups:
            attached_security_group_ids.append(sg['VpcSecurityGroupId'])
